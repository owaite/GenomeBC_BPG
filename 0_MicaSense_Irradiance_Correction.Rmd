...
Below, we have plotted the direct and horizontal irradiance from the DLS2 and values calculated with the DLS1 method [LINK]. We can see that the direct irradiance from the DLS2 is greater than the horizontal irradiance, which is physically impossible. In contrast, the direct irradiance from the DLS1 method is approximately 50 W/m²/nm less than the horizontal irradiance. 
...
```
From this graph, we would recommend correcting the data using the DLS1 method outlined below [LINK].
...
2) You have created a backup of your original data, since you will be irreversibly overwriting the exif data of the imagery. Prior to running the code below we copied all images in our Micasense folder to a folder in the same directory named Micasense_cor. Micasense_cor will be the folder we will be editing, and the Micasense folder will contain the unedited imagery

3) You have downloaded the MicaSense.config from the GitHub page ???????????????????????????????????????????????????????
...
xmp_corrected = readRDS(paste0(dir, date, "\\", MS_folder_name,"\\CSV\\",date,"_xmp_corrected.rds")) %>% #path to corrected xmp data .rds file
  mutate(SourceFile = str_replace(SourceFile,original_directory, corrected_directory), # Changing source file name to reflect where it will be on the SSD (working off the Q)
         # TargetFile = str_replace(SourceFile, ext_save, "")
         TargetFile = SourceFile) # current SourceFile is for MicaSense_Cleaned_save, here we are removing the "_save" so we just 

#fix sourcefile to run on E                                            # call the MicaSense_Cleaned folder, this is the folder of images whose meta data will get edited
# As vectors
...
for (i in seq_along(targets)) {
  ...
  call = paste0("-config G:/GitHub/SNC-GitHub/MicaSense/MicaSense.config", # SET PATH to the config file, this file is on the PARSER GitHub in the same folder as this R script
