...
ttops <- st_read("path to your grid")
ttops<- ttops%>%
  st_centroid()
...
To generate supercells we use a CHM, MicaSense orthomosaic, and P1 orthomosaic from similar dates
...
Figure @\ref(fig:seg-parts) shows the supercells created above in green. These are locations where
...
Figure @\ref(fig:seg-intersection) shows the ????????????????????????????
