...
DJI thermal's SDK requires inputs of humidity, reflection (or ambient temperature), distance, and emissivity (see [## DJI Thermal SDK: temperature conversion] for more detail on each of the requirements). In this section we will take hourly weather data from an on-site HOBO climate logger and calculate weighted averages for humidity and ambient temperature.
...
- read in the binary data outputted by the thermal SDK

- convert to a raster

- update new temperature raster with exif data from original _T.JPG H20T images


https://community.rstudio.com/t/solution-to-reading-raw-images-into-r/45435
...
for (i in 1:length(raw_list)){
  ...
  if (file.exists(test_exists)){
    ...
    extent(rast) <- extent(r_h20t)# the extent is bound by the resolution that you have assigned to it - so must change extent then resolution
    res(rast) <- res(r_h20t)
    #crs(rast) <- crs(r_h20t)
    ...
    #output are temperature tiffs with same exif data as original h20T images
  }
...
time.taken #time taken for script to run
```


Below in Figure @\ref(fig:crown-temp)

```{r crown-temp, echo=FALSE, fig.show="hold",fig.align='center', out.width="50%", fig.cap = "Left: temperature values within an individual crown from a flight on July 27th, 2022. Right: plot of hourly ambient tempatures from an on-site HOBO climate logger for July 27th, 2022, the grey vertical line is the time of the flight"}
