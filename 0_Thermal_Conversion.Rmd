...
Figure \@ref(fig:2-crown-temp-NIR) shows the temperature and NIR values across two mature (~25 year old) Douglas-fir tree crowns, where the crowns delineate the upper 25th percentile of the tree, from a flight that occurred on July 27th, 2022 under heatdome conditions. These flights occurred in full sun conditions. Looking at the NIR plots, we can see shadowed areas (see chapter \@ref(shadow-mask) for shadow masking with NIR) towards the north east sides of both crowns, which roughly aligns with the areas of cooler temperature seen in the crown temperature plots. As you can see in the daily temperature plot on the left, the MicaSense flight occurred from 1:30-2:30pm and was followed by the thermal flight, flown with the H20T, from roughly 2:40-3:40pm. Hence, we see a greater area of cooler crown temperatures than the NIR proxy for shadow would suggest, however this is likely due to the increase in shadow from the MicaSense flight to the thermal flight as the solar zenith angle decreases into the afternoon. 



```{r 2-crown-temp-NIR, echo=FALSE, fig.align='center', out.width="100%", fig.cap = "Left: plot of hourly ambient tempatures from an on-site HOBO climate logger at Canoe for July 27th, 2022. Middle: near-infrared (NIR) values for two Douglas-fir crowns aquired from a MicaSense flight flown directly before the thermal flight. Right: temperature values within two Douglas-fir crowns flown under heatdome conditions."}
...
Figure \@ref(fig:crown-temp-RGB) shows the temperature values for two young (~5 year old) Douglas-fir trees from an overcast day. Whereas we saw higher temperatures in areas with more direct light in Figure \@ref(fig:2-crown-temp-NIR), we see the opposite pattern below This is likely due to the heat of the ground radiating (i.e. warming) the lower branches of the young trees which are in view given the relatively small crown sizes and lack of crown closure. Whereas in Figure \@ref(fig:2-crown-temp-NIR) the Douglas-fir are mature and from a site that has reached crown closure, whcih resulted in the crowns used for metric calculations to be representative of the visible foliage from the top 25th percentile of the tree only.

We show these two examples, one of young trees in Figure \@ref(fig:crown-temp-RGB) and the other of mature trees in Figure \@ref(fig:2-crown-temp-NIR) to show both the radiometric resolution of the H20T within the crowns as well as some important factors to keep in mind when interpreting crown temperatures.
