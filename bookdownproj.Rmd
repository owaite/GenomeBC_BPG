...
<!--chapter:end:0_Introduction.Rmd-->
...
Now that we understand the reason behind the cyclical pattern of the sun sensor angles, we can use the solar zenith value, plotted as the horizontal black line in Figure \@ref(fig:SSA-DSL2-data), to make an informed decision on whether the values need correcting.

To help this decision we recommend plotting the DLS2 sun sensor values and the sun sensor values calculated with the DLS1 method which will be referred to as the "corrected" values, see the section on [sun sensor angle correction](#SSA-correction) for the correction method.
...
First, let's define direct and horizontal irradiance with respect to the DLS:
...
### Calculating Sun-Sensor Angles {#SSA-correction}
...
Plotting original verse corrected irradiance exif data to compare values from the DLS2 to corrected values using the DLS1 method (Figure \@ref(fig:DLS1-vs-DLS2-irr)).
...
Refer to the section [Is Correction Needed?](#is-correction-needed) to help decide whether your data needs correcting.
