...
Pixels containing shadows and openings in the canopy can introduce error in vegetation indices as reflectance values are often lower in shaded pixels or due to background noise (Malenovský et al., 2013; Zhang et al., 2024).  Masking out shaded pixels and openings within the crown is necessary to calculate accurate vegetation indices. We found masking with NIR reflectance to be a consistent and effective approach for masking out shadows and gaps. We use the NIR reflectance band (842nm) from the MicaSense MX Dual. NIR thresholds are determined using the shape of the NIR distribution. If the NIR distribution is bimodal, the threshold is set to the local minima, whereas if the distribution is unimodal, the threshold is set to the local maxima (Chen et al., 2007; D’Odorico et al., 2021; Otsu et al., 2019).

Below we define a function *find_local_min* that takes a list of first derivatives and returns a dataframe containing:

- **neg_value**: The value of the first derivative directly before the minima (i.e. the negative derivative value to the left of the minima)

- **pos_value**: The value of the first derivative directly after the minima (i.e. the positive derivative value to the right of the minima)

-	**pos_def**: The summation of the positive gradient values (a.k.a. slope values) in the first 15 gradient values following the local minimia. This value provides an idea of how great the increase in slope is after the local minima.

-	**neg_def**: The summation of the negative gradient values in the first 15 gradient values preceding the local minimia. This value provides an idea of how great the decrease in slope is before the local minima.

-	**Index**: Index position of the negative gradient value bordering the local minimia. This is the index of our threshold value since we do not get a true zero slope value at the local minimia but rather a change from a very small negative gradient value to a very small positive gradient value.

-	**definition**: The summation of the absolute value of pos_def and neg_def values. The greater this number, the more defined the minima. 
The above parameters were added to describe each minima found by the function so that the true local minima could be filtered for.

This function will be used in the next few steps to isolate minimas in the NIR distribution.
...
Next, the NIR band is selected from the multispectral orthomosaic, cropped to the extent of the crowns shapefile, and reformated from a raster to a vector of values:
...
At this point the threshold can be estimated manually from the graph however, this is not a feasible method when you have many data acquisitions, nor is it the most accurate method. Below we describe the method we used to create shadow masks for many acquisitions withouth having to visually inspect the NIR distributions. To computationally find the threshold we first need to decipher whether the NIR distribution is unimodal (one peak) or multimodal (more than one peak). To do so we use the *is.multimodal* function from the LaplacesDemon package if an *if else* statement. 

If the distribution was multimodal, we found the local minimia by:

1) calculating the first derivatives fo the NIR density values and storing the derivatives in dy_dt

2) applying the *find_local_min* function to the list of first derivatives to find minimas

3) filtering out small minimas that are not true minimas but rather dips on a larger slope

4) Identifying false multimodals and assigning the threshold value to be the local maxima

5) Identifying the largest minima (a.k.a. the local minima) and setting that as the threshold value

If the distribution was not multimodal (and therefore unimodal), we found the local maxima by:

1) locating the largest density values in the NIR distribution

2) finding the NIR value that that desnity value corresponds to and setting that NIR value as the threshold
...
Nest, we plot a histogram of the NIR values and annotate it to contain:
...
Here is an example of what this histogram will look like:
